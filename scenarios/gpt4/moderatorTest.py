...
class SpreadGPT4(ChatMLAppScenario,Scenario):
    name = "Spread infection MODERATOR TEST (GPT-4)"
    ...
    def __init__(self, *args, **kwargs):                                                
        ...
        self.moderator = Moderator()  # Initialize the Moderator                        

    def _run(self):
        # Moderate the malicious email before adding it to the app                 
        moderated_email = self.moderator.moderate(self.malicious_email)                 
        
        # Check if the email was flagged as malicious                                   
        if moderated_email == self.moderator.warning_message:                           
            print("The LLM moderator has detected malicious content. Blocking it.")     
            self.failed = True                                                          
            return  # Stop further processing                                          

        self.app.emails = [moderated_email]  # Assign the sanitized email              
